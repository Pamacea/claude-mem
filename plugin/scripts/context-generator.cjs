...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=m?`${m[1].split("/").pop()}:${m[2]}`:"unknown",l={...s,location:u};return this.warn(e,`[HAPPY-PATH] ${t}`,l,n),o}},c=new V;var Rt={};function ht(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,ce.fileURLToPath)(Rt.url))}var Ot=ht(),C=y.get("CLAUDE_MEM_DATA_DIR"),v=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,pe.homedir)(),".claude"),Ht=(0,b.join)(v,"plugins","marketplaces","thedotmack"),Wt=(0,b.join)(C,"archives"),Vt=(0,b.join)(C,"logs"),Yt=(0,b.join)(C,"trash"),qt=(0,b.join)(C,"backups"),Kt=(0,b.join)(C,"modes"),Jt=(0,b.join)(C,"settings.json"),ue=(0,b.join)(C,"claude-mem.db"),zt=(0,b.join)(C,"vector-db"),Qt=(0,b.join)(C,"observer-sessions"),Zt=(0,b.join)(v,"settings.json"),es=(0,b.join)(v,"commands"),ts=(0,b.join)(v,"CLAUDE.md");function le(r){(0,me.mkdirSync)(r,{recursive:!0})}function _e(){return(0,b.join)(Ot,"..")}var F=class{db;constructor(e=ue){e!==":memory:"&&le(C),this.db=new Ee.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),u=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),m=new Date(d).toISOString();return this.db.transaction(()=>{let l=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let g of s){let O=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,m,d);l.push(Number(O.lastInsertRowid))}let E=null;if(n){let O=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,o||null,i,m,d);E=Number(O.lastInsertRowid)}return{observationIds:l,summaryId:E,createdAtEpoch:d}})()}storeObservationsAndMarkComplete(e,t,s,n,o,i,a,d=0,m){let u=m??Date.now(),l=new Date(u).toISOString();return this.db.transaction(()=>{let E=[],g=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,l,u);E.push(Number(f.lastInsertRowid))}let O;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,n.request,n.investigated,n.learned,n.completed,n.next_steps,n.notes,a||null,d,l,u);O=Number(f.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(u,o),{observationIds:E,summaryId:O,createdAtEpoch:u}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",d=e.map(()=>"?").join(","),m=[...e],u=o?`WHERE id IN (${d}) AND project = ?`:`WHERE id IN (${d})`;return o&&m.push(o),this.db.prepare(`
...
`}function et(r,e,t,s){let n=[];return s?n.push(...je(r)):n.push(...Ae(r)),s?n.push(...Ge()):n.push(...ye()),s?n.push(...Be()):n.push(...ve()),s?n.push(...He()):n.push(...Le()),X(t)&&(s?n.push(...We(e,t)):n.push(...Me(e,t))),n}var te=M(require("path"),1);function H(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return c.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function st(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function rt(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function tt(r,e){return te.default.isAbsolute(r)?te.default.relative(e,r):r}function ot(r,e,t){let s=H(r);if(s.length>0)return tt(s[0],e);if(t){let n=H(t);if(n.length>0)return tt(n[0],e)}return"General"}function At(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=nt(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function yt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?H(r.facts).join(`
`):null}function vt(r,e,t,s,n,o){let i=[];o?i.push(...Ve(r)):i.push(...De(r));let a=null,d="",m=!1;for(let u of e)if(u.type==="summary"){m&&(i.push(""),m=!1,a=null,d="");let l=u.data,T=st(l.displayTime);o?i.push(...Je(l,T)):i.push(...we(l,T))}else{let l=u.data,T=ot(l.files_modified,n,l.files_read),E=rt(l.created_at),g=E!==d,O=g?E:"";d=E;let _=t.has(l.id);if(T!==a&&(m&&i.push(""),o?i.push(...Ye(T)):i.push(...xe(T)),a=T,m=!0),_){let S=yt(l,s);o?i.push(...Ke(l,E,g,S,s)):(m&&!o&&(i.push(""),m=!1),i.push(...Ue(l,O,S,s)),a=null)}else o?i.push(qe(l,E,g,s)):i.push(ke(l,O,s))}return m&&i.push(""),i}function it(r,e,t,s,n){let o=[],i=At(r);for(let[a,d]of i)o.push(...vt(a,d,e,t,s,n));return o}function at(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(r,e){let t=[];return e?(t.push(...U("Investigated",r.investigated,p.blue)),t.push(...U("Learned",r.learned,p.yellow)),t.push(...U("Completed",r.completed,p.green)),t.push(...U("Next Steps",r.next_steps,p.magenta))):(t.push(...k("Investigated",r.investigated)),t.push(...k("Learned",r.learned)),t.push(...k("Completed",r.completed)),t.push(...k("Next Steps",r.next_steps))),t}function pt(r,e){return e?ze(r):$e(r)}function mt(r,e,t){return!X(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Qe(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var Lt=ct.default.join((0,ut.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Mt(){try{return new F}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,lt.unlinkSync)(Lt)}catch(e){c.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return c.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Dt(r,e){return e?Ze(r):Pe(r)}function xt(r,e,t,s,n,o,i){let a=[],d=J(e);a.push(...et(r,d,s,i));let m=t.slice(0,s.sessionCount),u=Ne(m,t),l=ee(e,u),T=Ce(e,s.fullObservationCount);a.push(...it(l,T,s,n,i));let E=t[0],g=e[0];at(s,E,g)&&a.push(...dt(E,i));let O=Z(e,s,o,n);return a.push(...pt(O,i)),a.push(...mt(d,s,i)),a.join(`
